// https://github.com/FriendlyCaptcha/friendly-captcha-react/blob/main/src/components/Friendly-captcha/Frontend/useFriendlyCaptcha.tsx
import React, {
  Dispatch,
  ReactElement,
  SetStateAction,
...
type WidgetProps = React.HTMLAttributes<HTMLDivElement> & {
...
const FriendlyCaptcha = (props: FriendlyCaptchaWidgetProps) => {
  ...
  return (
    <>
      {!props.showAttribution && <style>{'.frc-banner { display: none }'}</style>}
...
function useFriendlyCaptcha({
  ...
  CaptchaWidget: (props: WidgetProps, customWidgetStyle?: CustomWidgetStyle) => ReactElement
  ...
  const solvedHandler = useCallback((solution: string) => {
    if (debug) {
  ...
  const errorHandler = useCallback((error: FriendlyServerErrorResponse) => {
  ...
  return useMemo(
    () => ({
      CaptchaWidget: (widgetProps: WidgetProps, customWidgetStyle?) => {
        return (
          <FriendlyCaptcha
            siteKey={siteKey}
            endpoint={endpoint}
    ...
    [siteKey, endpoint, language, startMode, showAttribution, debug],
  )
}

export { useFriendlyCaptcha }
